...
namespace PenaltyShootout.Stage0.Editor
{
    ...
    public static class Stage4ProjectBuilder
    {
        public const string ScenePath = "Assets/PenaltyShootout/Scenes/RobustTraining.unity";
        public const string PrefabPath =
            "Assets/PenaltyShootout/Prefabs/Stage4RobustTrainingArena.prefab";
        private const int TrainingArenaCount = 16;
        private const float TrainingArenaSpacing = 30f;

        [MenuItem("Penalty Shootout/Stage 4/Prepare Robust Training")]
        public static void PrepareProject()
        {
            EnsureStage2ArenaPrefab();
            ExportGoalkeeperRobustManifest();
            CreateStage4ArenaPrefab();
            CreateRobustTrainingScene();
            SetBuildScenes();
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            Debug.Log($"Prepared {ScenePath} and {PrefabPath} for Stage 4.");
        }

        private static void EnsureStage2ArenaPrefab()
        {
            if (AssetDatabase.LoadAssetAtPath<GameObject>(Stage2ProjectBuilder.PrefabPath) == null)
            {
                Stage2ProjectBuilder.PrepareProject();
            }
        }

        [MenuItem("Penalty Shootout/Stage 4/Build macOS Headless")]
        public static void BuildMacHeadless()
        {
            PrepareProject();
            var output = Path.GetFullPath(
                Path.Combine(Application.dataPath, "../../builds/macos/PenaltyShootoutStage4.app"));
            BuildHeadless(BuildTarget.StandaloneOSX, output);
        }

        [MenuItem("Penalty Shootout/Stage 4/Build Linux Headless")]
        public static void BuildLinuxHeadless()
        {
            PrepareProject();
            var output = Path.GetFullPath(
                Path.Combine(Application.dataPath, "../../builds/linux/PenaltyShootoutStage4.x86_64"));
            BuildHeadless(BuildTarget.StandaloneLinux64, output);
        }

        private static void ExportGoalkeeperRobustManifest()
        {
            var output = Path.GetFullPath(
                Path.Combine(Application.dataPath, "../../configs/environment/goalkeeper-robust-v0.json"));
            Directory.CreateDirectory(Path.GetDirectoryName(output)!);
            File.WriteAllText(output, KernelManifestUtility.CreateGoalkeeperRobustJson());
        }

        private static void CreateStage4ArenaPrefab()
        {
            if (!AssetDatabase.CopyAsset(Stage2ProjectBuilder.PrefabPath, PrefabPath) &&
                AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPath) == null)
            {
                throw new InvalidOperationException($"Failed to create {PrefabPath}.");
            }
            ...
            try
            {
                ConfigureStage4Arena(root.GetComponent<PenaltyAreaController>());
                PrefabUtility.SaveAsPrefabAsset(root, PrefabPath);
            }
            finally
            {
        ...
        private static void CreateRobustTrainingScene()
        {
            var arenaPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(PrefabPath);
            if (arenaPrefab == null)
            {
            ...
            for (var index = 0; index < TrainingArenaCount; index++)
            {
                ...
                if (instance == null)
                {
                    throw new InvalidOperationException("Failed to instantiate Stage 4 arena.");
                }

                instance.name = $"Stage4RobustTrainingArena_{index:000}";
                instance.transform.position =
                    new Vector3(column * TrainingArenaSpacing, 0f, row * TrainingArenaSpacing);
                var controller = instance.GetComponent<PenaltyAreaController>();
                ConfigureStage4Arena(controller);
                controller.ArenaId = index;
                controller.ShowDebugUi = false;
            }
        ...
        private static void ConfigureStage4Arena(PenaltyAreaController controller)
        {
            if (controller == null)
            {
                throw new InvalidOperationException("Stage 4 arena is missing PenaltyAreaController.");
            }
            ...
            if (agent == null || behavior == null)
            {
                throw new InvalidOperationException("Stage 4 arena is missing its ML-Agents behavior.");
            }

            agent.Controller = controller;
            agent.ObservationProfile = GoalkeeperObservationProfile.StatePartialV0;
            behavior.BehaviorName = KernelConstants.GoalkeeperRobustBehaviorName;
            behavior.BehaviorType = BehaviorType.HeuristicOnly;
            behavior.BrainParameters.VectorObservationSize =
                KernelConstants.GoalkeeperStateObservationSize;
            behavior.BrainParameters.NumStackedVectorObservations = 1;
        ...
        private static void SetBuildScenes()
        {
            EditorBuildSettings.scenes = new[]
            {
                new EditorBuildSettingsScene(Stage0ProjectBuilder.ScenePath, true),
                new EditorBuildSettingsScene(Stage1ProjectBuilder.ScenePath, true),
                new EditorBuildSettingsScene(Stage2ProjectBuilder.ScenePath, true),
                new EditorBuildSettingsScene(ScenePath, true),
            };
        }
