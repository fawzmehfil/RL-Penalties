...
"$UNITY" \
  ...
  -logFile "$PROJECT_ROOT/docs/stage5-imitation-prepare.log"
...
"$UNITY" \
  ...
  -testResults "$PROJECT_ROOT/docs/stage5-imitation-editmode-results.xml" \
  -logFile "$PROJECT_ROOT/docs/stage5-imitation-editmode.log"
...
"$UNITY" \
  ...
  -testResults "$PROJECT_ROOT/docs/stage5-imitation-playmode-results.xml" \
  -logFile "$PROJECT_ROOT/docs/stage5-imitation-playmode.log"

arch -x86_64 .venv/bin/python -m pytest \
  python/tests/test_stage5_control.py \
  python/tests/test_stage5_imitation.py \
  -q
...
"$UNITY" \
  ...
  -logFile "$PROJECT_ROOT/docs/stage5-imitation-macos-build.log"
...
"$UNITY" \
  ...
  -quit \
  -projectPath "$PROJECT_ROOT/unity" \
  -executeMethod PenaltyShootout.Stage0.Editor.Stage5ProjectBuilder.BuildMacReactiveDemonstration \
  -logFile "$PROJECT_ROOT/docs/stage5-imitation-demo-macos-build.log"

echo "Stage 5.5 imitation verification and builds passed."
